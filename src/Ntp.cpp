...
void ICACHE_FLASH_ATTR NtpClient::Ntp(const char * server, uint tz, time_t syncSecs) {
...
time_t ICACHE_FLASH_ATTR NtpClient::getNtpTime() {
	...
	if (udpListener.connect(timeServer, 123)) {
		udpListener.onPacket([](AsyncUDPPacket packet) {
			...
			time_t UnixUTCtime = (highWord << 16 | lowWord) - 2208988800UL + timezone;
			LOG__TRACE_F("NTP UnixUTCtime: %s (+ timezone: %s)", String(UnixUTCtime).c_str(), String(timezone).c_str());
			setTime(UnixUTCtime);
		});
	}
	else {
