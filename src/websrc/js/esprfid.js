...
function uncommited() {
    ...
    document.getElementById("commit").innerHTML = "<h6>شما تغییرات ذخیره‌نشده‌ای دارید؛ برای مشاهده و ذخیره تغییرات کلیک کنید!.</h6>";
...
function savegeneral() {
    ...
    if (a === null || a === "") {
        alert("کلمه عبور مدیر نمی‌تواند خالی باشد!");
        return;
    }
...
function checkOctects(input) {
    ...
    if (call.value.match(ipformat)) {
        ...
        alert("شما آدرس اشتباهی را در این ورودی وارد نموده‌اید: " + input);
        call.focus();
        return false;
    }
        ...
        if (statusTxt === "success") {
            ...
            var prg = setInterval(function() {
                ...
                if (i === 101) {
                    ...
                    a.innerText = "تلاش برای اتصال مجدد به برد";
                    ...
                    document.getElementById("updateprog").innerHTML = "تکمیل شده";
                }
            }, 500);
            switch (callback) {
                case "upload":
...
function getnextpage(mode) {
    if (!backupstarted) {
        document.getElementById("loadpages").innerHTML = "در حال بارگذاری " + page + "/" + haspages;
    }
...
function listStats() {
    ...
    document.getElementById("cpu").innerHTML = ajaxobj.cpu + " مگاهرتز";
    document.getElementById("uptime").innerHTML = ajaxobj.uptime;
    document.getElementById("heap").innerHTML = ajaxobj.heap + " بایت";
    document.getElementById("heap").style.width = (ajaxobj.heap * 100) / 40960 + "%";
    colorStatusbar(document.getElementById("heap"));
    document.getElementById("flash").innerHTML = ajaxobj.availsize + " بایت";
    ...
    document.getElementById("spiffs").innerHTML = ajaxobj.availspiffs + " بایت";
...
function backupset() {
    ...
    dlAnchorElem.setAttribute("download", "hut-duckling-settings.json");
    dlAnchorElem.click();
}
...
function piccBackup(obj) {
    ...
    dlAnchorElem.setAttribute("href", dataStr);
    dlAnchorElem.setAttribute("download", "hut-duckling-users.json");
    dlAnchorElem.click();
    backupstarted = false;
}
        ...
        var $modal = $("#editor-modal"),
            ...
            ft = window.FooTable.init("#usertable", {
                ...
                editing: {
                    ...
                    addRow: function() {
                        $editor[0].reset();
                        $editorTitle.text("اضافه کردن کاربر/استاد جدید");
                        $modal.modal("show");
                    },
                    editRow: function(row) {
                        var acctypefinder;
                        var values = row.val();
                        if (values.acctype === "همیشگی") {
                            acctypefinder = 1;
                        } else if (values.acctype === "مدیر") {
                            acctypefinder = 99;
                        } else if (values.acctype === "غیرفعال") {
                            acctypefinder = 0;
                        }
                    ...
                    deleteRow: function(row) {
                        ...
                        if (confirm("با این اقدام کاربر " + uid + " : " + username + " حذف خواهد شد؛ اطمینان دارید?")) {
            ...
            case "userlist":
                if (page < haspages && obj.result === true) {
                    ...
                    if (!backupstarted) {
                        ...
                        file.type = "hut-duckling-userbackup";
                        file.version = "v0.6";
                        file.list = data;
                        piccBackup(file);
                    }
...
window.FooTable.MyFiltering = window.FooTable.Filtering.extend({
    construct: function(instance) {
        ...
        this.acctypesstr = ["همیشگی", "مدیر", "غیرفعال"];
